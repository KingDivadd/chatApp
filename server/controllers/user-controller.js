...
const { BadRequestError, AuthorizationError, NotFoundError } = require('../error')
...
const getUsers = asyncHandler(async(req, res) => {
    ...
    res.status(StatusCodes.OK).json({ nbHIts: getUsers.length, getUsers })
})

const deleteManyUser = asyncHandler(async(req, res) => {
    const { userIds } = req.body
    const user = await User.deleteMany({ name: { $in: userIds } })
    if (!user) {
        res.status(StatusCodes.EXPECTATION_FAILED).json({ msg: "Unable to delete multiple users" })
    }
    res.status(StatusCodes.OK).json({ msg: "Multiple users deleted successfully...", user })
})
...
module.exports = { registerUser, authUser, getAllUser, getUsers, findForgetPassUser, updateUserName, updatePicture, deleteManyUser }