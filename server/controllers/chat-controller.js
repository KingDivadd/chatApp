...
const fetchChat = asyncHandler(async(req, res) => {
    var findChat = await Chat.find({
        ...
        // .populate({ path: "chatName", match: { "ChatName": { $ne: reqser } }, select: "userName" })
        // .populate({ path: "chatName", match: { isGroupChat: { $eq: true } }, select: "userName" })
        // .populate("chatPicture", "picture")
...
const updateGroupPicture = asyncHandler(async(req, res) => {
    ...
    const editPicture = await Chat.findOneAndUpdate({ _id: groupId }, { chatPicture }, { new: true, runValidators: true })
        .populate("groupAdmin", "-password")
        .populate("users", "-password")
    if (!editPicture) {
        res.status(StatusCodes.BAD_REQUEST).json({ msg: "Unable to change group name" })
    }
    res.status(StatusCodes.OK).json(editPicture)
})
...
const addUserToGroup = asyncHandler(async(req, res) => {
    ...
    if (chat.length > 0) {
        res.json({ msg: "User already present", chat: chat })
    } else {
        ...
        res.status(StatusCodes.OK).json({ msg: `adding user ${userID}`, users: added.users.length, chat: added })
...
const removeUserFromGroup = asyncHandler(async(req, res) => {
    ...
    res.status(StatusCodes.OK).json({ users: removed.users.length, chat: removed })
})
...
module.exports = { singleChat, fetchChat, deleteChat, groupChat, updateGroupName, updateGroupPicture, addUserToGroup, removeUserFromGroup }